...
void main( ){
   ...
   float p10 = texture2D( iChannel1, vUv - e.zy ).x;
   float p01 = texture2D( iChannel1, vUv - e.xz ).x;
   float p21 = texture2D( iChannel1, vUv + e.xz ).x;
   float p12 = texture2D( iChannel1, vUv + e.zy ).x;
